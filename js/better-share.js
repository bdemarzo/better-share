function betterShare() {
	...
	this.getImage = async function () {
		var dataUrl, filename, imagename = this.button.data('share-image-name');

		if (this.button.data('share-image-id')) {
			var image = document.getElementById(this.button.data('share-image-id'));
			switch (image.tagName.toLowerCase()) {
				case 'canvas':
					dataUrl = image.toDataURL();
					filename = imagename ?? image.id;
				...
				case 'img':
					dataUrl = image.src;
					filename = dataUrl.substring(dataUrl.lastIndexOf('/') + 1);
					break;
				default:
					console.error('data-share-image-id unsupported tag type: ' + image.tagName);
			}
		} else if (this.button.data('share-image-url')) {
			dataUrl = this.button.data('share-image-url');
			filename = dataUrl.substring(dataUrl.lastIndexOf('/') + 1);
		}
		if (dataUrl) {
